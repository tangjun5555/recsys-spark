 ...
 * 定义FM超参数
 */
trait FactorizationMachinesParams
//  extends PredictorParams with HasStepSize
  extends HasStepSize
   ...
   * @group param
   */
  final val algo: Param[Algo.Algo] = new Param[Algo.Algo](this, "algo", "The learning goal of factorization machines",
    ParamValidators.inArray(Array(Algo.Regression, Algo.BinaryClassification)))

  /**
   * @group getParam
   */
  final def getAlgo: Algo.Algo = $(algo)
   ...
   * @group param
   */
  final val solver: Param[Solver.Solver] = new Param[Solver.Solver](this, "solver", "The learning method of factorization machines",
   ...
   * @group getParam
   */
  final def getSolver: Solver.Solver = $(solver)
...
class FactorizationMachinesUpdater(
  ...
  override def compute(
    ...
    val brzRegParams = BDV.vertcat(BDV.fill(numFeatures * dim._3) {
      regParams._3
    },
      BDV.fill(numFeatures) {
        regParams._2
      }, BDV.fill(1) {
        regParams._1
      })
...
class FactorizationMachinesPerCoordinateUpdater(
                                                 ...
                                                 betaBWV: (Double, Double, Double)
                                               )
  extends PerCoordinateUpdater {
  override def compute(
                        weightsOld: MLlibVector,
                        gradient: MLlibVector,
                        alpha: Double,
    ...
    val brzRegParamsL1 = BDV.vertcat(BDV.fill(numFeatures * dim._3) {
      regParamsL1._3
    },
      BDV.fill(numFeatures) {
        regParamsL1._2
    ...
    val brzRegParamsL2 = BDV.vertcat(BDV.fill(numFeatures * dim._3) {
      regParamsL2._3
    },
      BDV.fill(numFeatures) {
        regParamsL2._2
...
class FactorizationMachines(override val uid: String)
   ...
   * @group setParam
   */
  def setAlgo(value: Algo.Algo): this.type = set(algo, value)
   ...
   * @group setParam
   */
  def setSolver(value: Solver.Solver): this.type = set(solver, value)
