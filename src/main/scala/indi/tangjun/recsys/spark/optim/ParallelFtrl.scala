package indi.tangjun.recsys.spark.optim
  ...
  def runParallelFtrl(
    ...
    while (!converged && i <= numIterations) {
      ...
      val (sumWeights, sumRegVal, lossSum) = data.repartition(numParts)
        ...
        val sumWeights = w1.toDense + w2.toDense
//        w1.toDense
        val sumRegVal = rv1 + rv2
        (Vectors.fromML(sumWeights), sumRegVal, ls1 + ls2)}, aggregationDepth)
  ...
  private def isConverged(
    ...
    val previousBDV = previousWeights.toDense
    val currentBDV = currentWeights.toDense
