 ...
 * time: 2021/1/13 2:29 下午
 * description:
 */
class RecallAtKEvaluator extends Evaluator {
  ...
  override def evaluate(dataset: Dataset[_]): Double = {
    val t1 = dataset.select(labelColumnName, predictionColumnName)
      ...
      MathFunctionUtil.computeRecRecallRate(labels, predicts, k)
    })
  ...
  override val uid: String = Identifiable.randomUID("RecallAtKEvaluator")
