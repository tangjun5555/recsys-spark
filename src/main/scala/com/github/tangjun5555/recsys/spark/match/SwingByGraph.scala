 ...
 *
 */
class SwingByGraph extends I2IMatch with U2IMatch {

  private case class NodeAttr(
                               var nodeType: String = "",
                               var neighbors: Array[Long] = Array.empty[Long] // 顶点的邻居
                             ) extends Serializable

  private case class EdgeAttr(
                               var srcNeighbors: Array[Long] = Array.empty[Long], // 起点的邻居
                               var dstNeighbors: Array[Long] = Array.empty[Long] // 终点的邻居
                             ) extends Serializable
  ...
  def fit(rawDataDF: DataFrame): this.type = {
    ...
    val userId2IndexDF = dataDF.rdd.map(x => x.getAs[String](userColumnName))
      .distinct()
      .zipWithIndex()
      .toDF(userColumnName, userColumnName + "_index")
      .persist(StorageLevel.MEMORY_AND_DISK)
    userId2IndexDF.show(50, false)

    val itemId2IndexDF = dataDF.rdd.map(x => x.getAs[String](itemColumnName))
      .distinct()
      .zipWithIndex()
      .toDF(itemColumnName, itemColumnName + "_index")
      .persist(StorageLevel.MEMORY_AND_DISK)
    itemId2IndexDF.show(50, false)

    val inputTriplets: RDD[(VertexId, VertexId)] = dataDF
      .join(userId2IndexDF, Seq(userColumnName), "iner")
      .join(itemId2IndexDF, Seq(itemColumnName), "inner")
      .rdd.map(row => (row.getAs[Long](userColumnName + "_index"), row.getAs[Long](itemColumnName + "_index")))
      .persist(StorageLevel.MEMORY_AND_DISK)

    val graphNodes: RDD[(Long, NodeAttr)] =
      inputTriplets
        .map(x => (x._1, Array(x._2)))
        .reduceByKey(_ ++ _)
        .map(x => (x._1, NodeAttr(nodeType = "user", neighbors = x._2)))
        .union(
          inputTriplets
            .map(x => (x._2, Array(x._1)))
            .reduceByKey(_ ++ _)
            .map(x => (x._1, NodeAttr(nodeType = "item", neighbors = x._2)))
        )
        .cache()
    println(graphNodes.first())

    val graphEdges: RDD[Edge[EdgeAttr]] = graphNodes
      .filter(_._2.nodeType.equals("user"))
      .flatMap { case (srcId, nodeAttr) =>
        nodeAttr.neighbors.map { dstId =>
